...
def main():
    ...
    if result.get("skipped"):
        print(f"鈿?Skipped duplicate (already stored for user {result['user_id']})")
    elif result["success"]:
        print(f"鉁?Stored for user '{result['user_id']}' turn {result['turn_number']}")
        print(f"   Conversation: {result['conversation_id'][:8]}...")
    else:
        print("鉂?Failed to store conversation", file=sys.stderr)
        sys.exit(1)
