...
def get_embedding(text: str) -> Optional[List[float]]:
    data = json.dumps({"model": "mxbai-embed-large", "input": text[:8192]}).encode()
    req = urllib.request.Request(f"http://127.0.0.1:11434/v1/embeddings", data=data, headers={"Content-Type": "application/json"})
    try:
