...
def format_result(result, index):
    ...
    # Role emoji
    role_emoji = "馃懁" if role == "user" else "馃"
    
    # Source indicator
    source_icon = "馃敶" if source == "redis" else "馃數"
    ...
    if source == "qdrant" and result.get('ai_response'):
        ...
        if len(result['ai_response']) > 150:
            ai_resp += "..."
        lines.append(f"   馃挰 AI: {ai_resp}")
    
    if score != 'exact':
        lines.append(f"   馃搳 Score: {score}")
    else:
        lines.append(f"   馃搳 Match: exact (Redis)")
...
def main():
    ...
    if redis_sorted:
        print("馃敶 FROM REDIS (Recent Buffer):\n")
    ...
    # Then Qdrant results
    if qdrant_sorted:
        print("馃數 FROM QDRANT (Long-term Memory):\n")
    ...
    if redis_sorted:
        print(f"  馃敶 Redis: {len(redis_sorted)} (exact, recent)")
    if qdrant_sorted:
        print(f"  馃數 Qdrant: {len(qdrant_sorted)} (semantic, long-term)")
