...
CRON_CAPTURE_SCHEDULE="${CRON_CAPTURE_SCHEDULE:-5 11 * * *}"
TR_SCHEDULE="${TR_SCHEDULE:-30 11 * * *}"
BACKUP_SCHEDULE="${BACKUP_SCHEDULE:-0 12 * * *}"
SLIDING_SCHEDULE="${SLIDING_SCHEDULE:-30 12 * * *}"
...
update["commandTimeoutMs"] = max(int(update.get("commandTimeoutMs", 0) or 0), qmd_timeout_ms)
update["updateTimeoutMs"] = max(int(update.get("updateTimeoutMs", 0) or 0), qmd_timeout_ms)
update["embedTimeoutMs"] = max(int(update.get("embedTimeoutMs", 0) or 0), qmd_timeout_ms)
