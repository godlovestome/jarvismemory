...
class RuntimePathTests(unittest.TestCase):
    ...
    def test_docs_track_version_and_lossless_update(self) -> None:
        ...
        self.assertIn('Jarvis Memory + True Recall v2.1.0', readme)
    ...
    def test_bootstrap_configures_qmd_separately_from_true_recall(self) -> None:
        ...
        self.assertIn('qmd_root = Path("/home/openclaw/qmd-index")', text)
        self.assertIn('qmd["command"] = "/home/openclaw/scripts/qmd-openclaw-wrapper.sh"', text)
        self.assertIn('qmd["searchMode"] = qmd.get("searchMode") or "query"', text)
        self.assertIn('qmd["includeDefaultMemory"] = False', text)
        self.assertIn('qmd["paths"] = paths', text)
        self.assertIn('update["commandTimeoutMs"] = max(', text)
        self.assertIn('update["updateTimeoutMs"] = max(', text)
        self.assertIn('update["embedTimeoutMs"] = max(', text)
        ...
        self.assertIn('"name": "qmd-index"', text)
    ...
    def test_plugin_package_exists_with_manifest_and_entrypoint(self) -> None:
        ...
        self.assertIn('"version": "2.1.0"', package)
    ...
    def test_audit_reports_qmd_and_true_recall_as_independent_chains(self) -> None:
        ...
        self.assertIn('embedTimeoutMs', text)
        self.assertIn('command=', text)
        self.assertIn('searchMode=', text)
        self.assertIn('includeDefaultMemory=', text)
